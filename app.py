...
def text_1():
    ...
    #means['Agreeableness'] = np.mean(str(df_new['Agreeableness']))
    means['Conscientiousness'] = np.mean(df_new['Conscientiousness'])
    means['Openness'] = np.mean(df_new['Openness'])
    
    probas_others = [np.mean(df_new['Extraversion']), np.mean(df_new['Neuroticism']), np.mean(df_new['Conscientiousness']), np.mean(df_new['Openness'])]
